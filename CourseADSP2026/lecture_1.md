...
- [Notation and Variable Definitions](#notation-and-variable-definitions)
  - [Time, Frequency, and Index Variables](#time-frequency-and-index-variables)
    - [How to Interpret `rad/sample`](#how-to-interpret-radsample)
  - [Signals and Sequences](#signals-and-sequences)
  - [Transforms and Spectral Functions](#transforms-and-spectral-functions)
  - [Laplace and z-Domain Variables](#laplace-and-z-domain-variables)
  - [LTI System and Filter Parameters](#lti-system-and-filter-parameters)
  - [Allpass, Minimum-Phase, and Spectral Factorization](#allpass-minimum-phase-and-spectral-factorization)
  - [OFDM and Multipath Channel Model (§1.1.4)](#ofdm-and-multipath-channel-model-114)
- [Part I: Digital Signals and DSP Overview](#part-i-digital-signals-and-dsp-overview)
  - [1.1 Basic Types and Examples of Digital Signals](#11-basic-types-and-examples-of-digital-signals)
    - [1.1.1 Speech Signals](#111-speech-signals)
    - [1.1.2 Image Signals](#112-image-signals)
    - [1.1.3 Video Signals](#113-video-signals)
    - [1.1.4 Communication Signals](#114-communication-signals)
      - [How to Interpret a Multipath Channel Tap](#how-to-interpret-a-multipath-channel-tap)
    - [1.1.5 Radar Signals](#115-radar-signals)
  ...
  - [1.3 General Digital Signal Processing System](#13-general-digital-signal-processing-system)
    - [1.3.1 System Pipeline](#131-system-pipeline)
    - [1.3.2 Sampling Theorem and Bandlimited Condition](#132-sampling-theorem-and-bandlimited-condition)
    - [1.3.3 Bandpass Signals and the Bandpass Sampling Theorem](#133-bandpass-signals-and-the-bandpass-sampling-theorem)
    - [1.3.4 Spectral Translation: Up-Conversion and the Apparent Doubling of Bandwidth](#134-spectral-translation-up-conversion-and-the-apparent-doubling-of-bandwidth)
    - [1.3.5 The Two Fundamental Problems of DSP](#135-the-two-fundamental-problems-of-dsp)
  ...
  - [1.5 Elementary Sequences (Fundamental Building Blocks)](#15-elementary-sequences-fundamental-building-blocks)
    - [1.5.1 Unit Impulse $\delta(n)$](#151-unit-impulse-deltan)
    - [1.5.2 Unit Step $u(n)$](#152-unit-step-un)
    - [1.5.3 Real Exponential Sequence $\alpha^n u(n)$](#153-real-exponential-sequence-alphan-un)
    - [1.5.4 Complex Exponential Sequence $e^{j\omega_0 n}$](#154-complex-exponential-sequence-ejomega_0-n)
- [Part II: Transforms for Discrete-Time Signals](#part-ii-transforms-for-discrete-time-signals)
  - [2.1 Discrete-Time Fourier Transform (DTFT)](#21-discrete-time-fourier-transform-dtft)
    - [2.1.1 Definition and Physical Meaning](#211-definition-and-physical-meaning)
    - [2.1.2 Main Properties of the DTFT](#212-main-properties-of-the-dtft)
  - [2.2 z-Transform](#22-z-transform)
    - [2.2.1 Definition and Region of Convergence](#221-definition-and-region-of-convergence)
    - [2.2.2 Main Properties of the z-Transform](#222-main-properties-of-the-z-transform)
    - [2.2.3 Common z-Transform Pairs](#223-common-z-transform-pairs)
    - [2.2.4 Relationship Between the z-Transform and the Laplace Transform](#224-relationship-between-the-z-transform-and-the-laplace-transform)
  - [2.3 Discrete Fourier Transform (DFT)](#23-discrete-fourier-transform-dft)
    - [2.3.1 From DTFT to DFT: Uniform Frequency Sampling](#231-from-dtft-to-dft-uniform-frequency-sampling)
    - [2.3.2 Relationship Between DFT and DFS (Discrete Fourier Series)](#232-relationship-between-dft-and-dfs-discrete-fourier-series)
      - [Step 1 — The DFS: transform for periodic sequences](#step-1--the-dfs-transform-for-periodic-sequences)
      - [Step 2 — Periodic extension of a finite-length sequence](#step-2--periodic-extension-of-a-finite-length-sequence)
      - [Step 3 — DFT equals one period of the DFS](#step-3--dft-equals-one-period-of-the-dfs)
      - [Summary: DFT vs. DFS at a glance](#summary-dft-vs-dfs-at-a-glance)
      - [When Does Time-Domain Aliasing Occur?](#when-does-time-domain-aliasing-occur)
      - [Numerical Example: What Goes Wrong When $N$ Is Too Small](#numerical-example-what-goes-wrong-when-n-is-too-small)
    - [2.3.3 Main Properties and Uses of the DFT](#233-main-properties-and-uses-of-the-dft)
  - [2.4 Fast Fourier Transform (FFT)](#24-fast-fourier-transform-fft)
    - [2.4.4 Divide-and-Conquer Strategy and Twiddle Factor Properties](#244-divide-and-conquer-strategy-and-twiddle-factor-properties)
    - [2.4.5 Radix-2 DIT and DIF FFT](#245-radix-2-dit-and-dif-fft)
      - [Decimation-in-Time (DIT) FFT](#decimation-in-time-dit-fft)
      - [Decimation-in-Frequency (DIF) FFT](#decimation-in-frequency-dif-fft)
    - [2.4.1 How Much Faster Is FFT than Direct DFT on a CPU?](#241-how-much-faster-is-fft-than-direct-dft-on-a-cpu)
      - [CPU experiment: matrix DFT versus NumPy FFT](#cpu-experiment-matrix-dft-versus-numpy-fft)
    - [2.4.2 Is FFT Actually Deployed on CPUs in Real Systems?](#242-is-fft-actually-deployed-on-cpus-in-real-systems)
    - [2.4.3 When Can a GPU Greatly Accelerate FFT?](#243-when-can-a-gpu-greatly-accelerate-fft)
      - [Case 1: many independent transforms](#case-1-many-independent-transforms)
      - [Case 2: one sufficiently large transform](#case-2-one-sufficiently-large-transform)
      - [Case 3: the data already resides on the GPU](#case-3-the-data-already-resides-on-the-gpu)
      - [Why GPU FFT is not identical to neural-network acceleration](#why-gpu-fft-is-not-identical-to-neural-network-acceleration)
      - [GPU, FPGA, and DSP are all parallel](#gpu-fpga-and-dsp-are-all-parallel)
      - [Practical decision guide](#practical-decision-guide)
    - [2.4.6 Computational Complexity](#246-computational-complexity)
    - [2.4.7 Bit-Reversal Permutation](#247-bit-reversal-permutation)
- [Part III: Digital Filter Structures and Design](#part-iii-digital-filter-structures-and-design)
  - [3.0 Characterizing Discrete-Time Systems](#30-characterizing-discrete-time-systems)
    - [3.0.1 The Difference Equation — Computational Recipe for a Filter](#301-the-difference-equation--computational-recipe-for-a-filter)
    - [3.0.2 The Impulse Response — Complete Characterization of an LTI System](#302-the-impulse-response--complete-characterization-of-an-lti-system)
    - [3.0.3 The Frequency Response — What the Filter Does in the Frequency Domain](#303-the-frequency-response--what-the-filter-does-in-the-frequency-domain)
    - [3.0.4 FIR vs. IIR — The Fundamental Design Choice](#304-fir-vs-iir--the-fundamental-design-choice)
  - [3.1 FIR Filter Implementations](#31-fir-filter-implementations)
    - [3.1.1 Direct Form — Transversal Filter](#311-direct-form--transversal-filter)
    - [3.1.2 Cascade Form](#312-cascade-form)
    - [3.1.3 Recursive Realization and Comb Filter](#313-recursive-realization-and-comb-filter)
    - [3.1.4 Frequency-Sampling Form](#314-frequency-sampling-form)
  - [3.2 IIR Filter Implementations](#32-iir-filter-implementations)
    - [3.2.1 Direct Form I](#321-direct-form-i)
    - [3.2.2 Direct Form II — Canonical Form](#322-direct-form-ii--canonical-form)
    - [3.2.3 Cascade Form](#323-cascade-form)
    - [3.2.4 Parallel Form](#324-parallel-form)
  - [3.3 Digital Filter Design](#33-digital-filter-design)
    - [3.3.1 Ideal and Practical Frequency Responses](#331-ideal-and-practical-frequency-responses)
    - [3.3.2 FIR Filter Design: Window Method, Frequency-Sampling Method, Kaiser Formula](#332-fir-filter-design-window-method-frequency-sampling-method-kaiser-formula)
    - [3.3.3 IIR Filter Design: Impulse Invariance, Bilinear Transform, Frequency Transformation](#333-iir-filter-design-impulse-invariance-bilinear-transform-frequency-transformation)
- [Part IV: Special Sequences and Corresponding Filters](#part-iv-special-sequences-and-corresponding-filters)
  - [4.1 Allpass Sequences and Allpass Filters](#41-allpass-sequences-and-allpass-filters)
    - [4.1.1 Definition and Properties](#411-definition-and-properties)
    - [4.1.2 Group Delay of Allpass Filters — Proof That Group Delay is Always Positive](#412-group-delay-of-allpass-filters--proof-that-group-delay-is-always-positive)
  - [4.2 Minimum-Phase Sequences and Minimum-Phase Filters](#42-minimum-phase-sequences-and-minimum-phase-filters)
    ...
    - [4.2.2 Theorem 1.1: Minimum-Phase / Allpass Decomposition](#422-theorem-11-minimum-phase--allpass-decomposition)
  - [4.3 Linear-Phase Sequences and Linear-Phase Filters](#43-linear-phase-sequences-and-linear-phase-filters)
    - [4.3.1 Strict and Generalized Linear Phase](#431-strict-and-generalized-linear-phase)
    - [4.3.2 Four Types of Linear-Phase FIR Filters](#432-four-types-of-linear-phase-fir-filters)
    - [4.3.3 Applicable Filter Types per Class](#433-applicable-filter-types-per-class)
  - [4.4 Positive Semi-Definite Sequences](#44-positive-semi-definite-sequences)
    - [4.4.1 Autocorrelation Sequences and Power Spectral Non-Negativity](#441-autocorrelation-sequences-and-power-spectral-non-negativity)
...
- [1.1 Basic Types and Examples of Digital Signals](#11-basic-types-and-examples-of-digital-signals)
- <a href="#12-applications-of-digital-signal-processing" style="color: #a0a0a0;">1.2 Applications of Digital Signal Processing</a>
- <a href="#13-general-digital-signal-processing-system" style="color: #a0a0a0;">1.3 General Digital Signal Processing System</a>
- <a href="#14-review-of-undergraduate-dsp-course-structure" style="color: #a0a0a0;">1.4 Review of Undergraduate DSP Course Structure</a>
- <a href="#15-elementary-sequences-fundamental-building-blocks" style="color: #a0a0a0;">1.5 Elementary Sequences (Fundamental Building Blocks)</a>
...
- <a href="#11-basic-types-and-examples-of-digital-signals" style="color: #a0a0a0;">1.1 Basic Types and Examples of Digital Signals</a>
- [1.2 Applications of Digital Signal Processing](#12-applications-of-digital-signal-processing)
- <a href="#13-general-digital-signal-processing-system" style="color: #a0a0a0;">1.3 General Digital Signal Processing System</a>
- <a href="#14-review-of-undergraduate-dsp-course-structure" style="color: #a0a0a0;">1.4 Review of Undergraduate DSP Course Structure</a>
- <a href="#15-elementary-sequences-fundamental-building-blocks" style="color: #a0a0a0;">1.5 Elementary Sequences (Fundamental Building Blocks)</a>
...
- [2.1 Discrete-Time Fourier Transform (DTFT)](#21-discrete-time-fourier-transform-dtft)
- <a href="#22-z-transform" style="color: #a0a0a0;">2.2 z-Transform</a>
- <a href="#23-discrete-fourier-transform-dft" style="color: #a0a0a0;">2.3 Discrete Fourier Transform (DFT)</a>
- <a href="#24-fast-fourier-transform-fft" style="color: #a0a0a0;">2.4 Fast Fourier Transform (FFT)</a>
...
- <a href="#21-discrete-time-fourier-transform-dtft" style="color: #a0a0a0;">2.1 Discrete-Time Fourier Transform (DTFT)</a>
- [2.2 z-Transform](#22-z-transform)
- <a href="#23-discrete-fourier-transform-dft" style="color: #a0a0a0;">2.3 Discrete Fourier Transform (DFT)</a>
- <a href="#24-fast-fourier-transform-fft" style="color: #a0a0a0;">2.4 Fast Fourier Transform (FFT)</a>
...
- [3.0 Characterizing Discrete-Time Systems](#30-characterizing-discrete-time-systems)
- <a href="#31-fir-filter-implementations" style="color: #a0a0a0;">3.1 FIR Filter Implementations</a>
- <a href="#32-iir-filter-implementations" style="color: #a0a0a0;">3.2 IIR Filter Implementations</a>
- <a href="#33-digital-filter-design" style="color: #a0a0a0;">3.3 Digital Filter Design</a>
...
> **Why the stability conditions differ: poles, unit circle, and ROC**
>
...
- <a href="#30-characterizing-discrete-time-systems" style="color: #a0a0a0;">3.0 Characterizing Discrete-Time Systems</a>
- [3.1 FIR Filter Implementations](#31-fir-filter-implementations)
- <a href="#32-iir-filter-implementations" style="color: #a0a0a0;">3.2 IIR Filter Implementations</a>
- <a href="#33-digital-filter-design" style="color: #a0a0a0;">3.3 Digital Filter Design</a>
...
- [4.1 Allpass Sequences and Allpass Filters](#41-allpass-sequences-and-allpass-filters)
- <a href="#42-minimum-phase-sequences-and-minimum-phase-filters" style="color: #a0a0a0;">4.2 Minimum-Phase Sequences and Minimum-Phase Filters</a>
- <a href="#43-linear-phase-sequences-and-linear-phase-filters" style="color: #a0a0a0;">4.3 Linear-Phase Sequences and Linear-Phase Filters</a>
- <a href="#44-positive-semi-definite-sequences" style="color: #a0a0a0;">4.4 Positive Semi-Definite Sequences</a>
- <a href="#chapter-1-summary" style="color: #a0a0a0;">Chapter 1 Summary</a>
...
- <a href="#41-allpass-sequences-and-allpass-filters" style="color: #a0a0a0;">4.1 Allpass Sequences and Allpass Filters</a>
- [4.2 Minimum-Phase Sequences and Minimum-Phase Filters](#42-minimum-phase-sequences-and-minimum-phase-filters)
- <a href="#43-linear-phase-sequences-and-linear-phase-filters" style="color: #a0a0a0;">4.3 Linear-Phase Sequences and Linear-Phase Filters</a>
- <a href="#44-positive-semi-definite-sequences" style="color: #a0a0a0;">4.4 Positive Semi-Definite Sequences</a>
- <a href="#chapter-1-summary" style="color: #a0a0a0;">Chapter 1 Summary</a>
