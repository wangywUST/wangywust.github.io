...
**Why this governs LMS convergence (preview of Chapter 7).** Steepest-descent and LMS minimize the mean-square error over a quadratic error surface whose curvature in each principal direction is set by the eigenvalues of $\mathbf{R}_{x}$. The level sets are ellipsoids whose axis-length ratio is
...
**Question.** An acoustic engineer wants a simple statistical model of late reverberation in a room. Each new sound sample contains an independent unit-variance excitation $w(n)$ plus $80\%$ of the previous reverberant sample, so $x(n)=0.8x(n-1)+w(n)$. Equivalently, the room is modeled by the stable causal filter
...
An **MA($q$) process** is the special case $p=0$ of the ARMA model. With no autoregressive terms, the current output is a weighted sum of the current and previous $q$ white-noise samples:

$$\boxed{x(n) = w(n) + \sum_{k=1}^{q} b_k\, w(n-k) = B(z)\, w(n), \qquad w(n) \sim \mathrm{WN}(0, \sigma_w^2)}$$

More explicitly, define $b_0=1$. Then
...
$$
X(z)=\left(\sum_{k=0}^{q}b_kz^{-k}\right)W(z)
=B(z)W(z),
$$

so the white-noise shaping filter is

$$
\boxed{H(z)\triangleq\frac{X(z)}{W(z)}=B(z)
=1+\sum_{k=1}^{q}b_kz^{-k}.}
$$

This explains both parts of the phrase **all-zero (FIR) model**:

- **FIR:** its impulse response is $h(k)=b_k$ for $0\le k\le q$ and zero otherwise, so it has finite duration $q+1$;
- **all-zero:** $H(z)$ has no feedback denominator other than $1$, hence no nontrivial poles. Its adjustable frequency-selective features are determined by the $q$ zeros of $B(z)$.

Thus an AR model shapes white noise through feedback poles, whereas an MA model shapes it through a finite feedforward combination and its zeros.
...
### Why the MA Representation Is Not Unique
...
(a,\sigma_w^2)
\qquad\text{and}\qquad
\left(\frac1a,\,a^2\sigma_w^2\right)
$$
...
**Question.** An autonomous underwater vehicle forms a side-scan-sonar image of the seafloor. Two adjacent zero-mean pixel residuals are collected as $\mathbf x=[x_1,x_2]^T$; because neighboring pixels insonify nearly the same seabed patch, their calibrated covariance is
