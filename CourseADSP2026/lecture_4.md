...
| Symbol | Definition |
|--------|------------|
| $r_x(l)=E\lbrace x(n)x^\ast(n-l)\rbrace $ | Autocorrelation sequence (ACS) of a WSS process |
...
An underwater listening buoy samples one hydrophone for $1\,\text{s}$ at $48\,\text{kHz}$ with 16-bit samples. For long-term environmental monitoring it may store either the entire record or a ten-coefficient acoustic model plus one innovation-variance value, each model value represented by 32 bits. How many bytes does each representation require, what is the numerical storage-reduction factor, and what percentage of the original variance has the model explained if the measured signal variance is $100$ but the modeling residual variance is $25$?

The preceding lectures provide prediction machinery, but they have not yet organized it into the measurement–model–validation workflow or quantified why a compact stochastic signal model is useful. Section 0 establishes that viewpoint.
...
$$E\lbrace \lvert e(n)\rvert^2\rbrace .$$
...
|---------------------|----------------------------|
| Use $r_x(l)=E\lbrace x(n)x^\ast(n-l)\rbrace $ | Estimate $\hat r_x(l)$ from data |
| Minimize $E\lbrace \lvert e(n)\rvert^2\rbrace $ | Minimize $\sum_n \lvert e(n)\rvert^2$ |
...
**Question.** Compare raw and ten-coefficient storage for one second of 48-kHz, 16-bit hydrophone data, and quantify the variance explained when power falls from $100$ to $25$.

**Answer.**

The raw record contains $48{,}000$ samples, so it requires

$$48{,}000(16)=768{,}000\ \text{bits}=96{,}000\ \text{bytes}.$$

Ten coefficients plus one variance give eleven 32-bit values:

$$11(32)=352\ \text{bits}=44\ \text{bytes}.$$

The storage-reduction factor is therefore

$$\frac{96{,}000}{44}=2181.82.$$

The model removes $100-25=75$ variance units from the residual, so its explained fraction is $75/100=75\%$. **Numerically, the representations require $96{,}000$ bytes and $44$ bytes, the compact model is about $2182\times$ smaller, and it explains $75\%$ of the measured variance.** This compression does not mean the waveform can necessarily be reconstructed sample for sample; it means the chosen acoustic behavior has been summarized by a small parameter set and a residual whose adequacy must still be validated.
...
Ocean ambient noise of variance $2$ is passed through the causal acoustic coloring filter $H(z)=1/(1-0.6z^{-1})$ to simulate correlated hydrophone noise. Calculate the output variance, lag-one autocorrelation and correlation coefficient, PSD at $\omega=0$ and $\omega=\pi$, and the variance and PSD level obtained after applying the exact whitening filter.

The roadmap says that random signals may be generated by a model; §1 supplies the coloring, whitening, spectral-factor, and innovations calculations needed to predict the model's measurable statistics.
...
$$\lbrace a_1,\ldots,a_P,d_1,\ldots,d_Q,\sigma_w^2\rbrace .$$
...
$$\phi(\omega)=-\mathcal{H}\lbrace \log \lvert H(e^{j\omega})\rvert\rbrace .$$
...
$$\mathrm{SFM}_x=\frac{\exp\left\lbrace \frac{1}{2\pi}\int_{-\pi}^{\pi}\ln R_x(e^{j\omega})d\omega\right\rbrace }{\frac{1}{2\pi}\int_{-\pi}^{\pi}R_x(e^{j\omega})d\omega}.$$
...
$$\exp\left\lbrace \frac{1}{2\pi}\int_{-\pi}^{\pi}\ln R_x(e^{j\omega})d\omega\right\rbrace .$$
...
&\quad =
\sigma_w^2
\exp\left\lbrace \frac{1}{2\pi}\int_{-\pi}^{\pi}\ln \lvert H(e^{j\omega})\rvert^2d\omega\right\rbrace .
\end{aligned}
$$
...
$$\exp\left\lbrace \frac{1}{2\pi}\int_{-\pi}^{\pi}\ln R_x(e^{j\omega})d\omega\right\rbrace =\sigma_w^2.$$
...
**Question.** Unit-variance-$2$ ocean noise is colored by $H(z)=1/(1-0.6z^{-1})$. Find its output correlations, endpoint PSD values, and whitening result.

**Answer.**

The stationary output is AR(1). Its variance and lag-one autocorrelation are

$$r_x(0)=\frac{2}{1-0.6^2}=3.125,\qquad
r_x(1)=0.6r_x(0)=1.875,$$

so the lag-one correlation coefficient is $1.875/3.125=0.6$. Its PSD is

$$R_x(e^{j\omega})=\frac{2}{|1-0.6e^{-j\omega}|^2},$$

which gives

$$R_x(e^{j0})=\frac{2}{0.4^2}=12.5,\qquad
R_x(e^{j\pi})=\frac{2}{1.6^2}=0.78125.$$

The exact whitening filter is $A(z)=1-0.6z^{-1}$. Applying it recovers the innovation $w(n)$, whose variance is $2$ and whose PSD is the flat level $2$ at every frequency. **Numerically, the colored-noise variance is $3.125$, $r(1)=1.875$, $\rho(1)=0.6$, its endpoint PSDs are $12.5$ and $0.78125$, and whitening produces variance and flat PSD level $2$.**
...
A shallow-water echo-plus-reverberation model is

$$H(z)=\frac{1+0.5z^{-1}}{1-0.8z^{-1}},$$

driven by unit-variance white noise. Give its pole and zero, the first three impulse-response samples, and its PSD at $\omega=0$ and $\omega=\pi$. What is the numerical ratio of those two PSD values, and is the causal model stable and minimum phase?

Section 1 treats a general coloring filter, but it does not yet show how a small set of poles and zeros separately controls resonance, notches, memory, and invertibility. Section 2 develops that parametric view.
...
**Question.** For $H(z)=(1+0.5z^{-1})/(1-0.8z^{-1})$, find its geometry, first impulse samples, endpoint PSDs, ratio, stability, and minimum-phase status.

**Answer.**

The denominator zero gives a pole at $z=0.8$, and the numerator gives a zero at $z=-0.5$. Expanding the denominator,

$$H(z)=(1+0.5z^{-1})\sum_{n=0}^{\infty}0.8^nz^{-n},$$

so the first three samples are

$$h(0)=1,\qquad h(1)=0.8+0.5=1.3,\qquad h(2)=0.8^2+0.5(0.8)=1.04.$$

With unit innovation variance,

$$R_x(e^{j0})=\frac{|1+0.5|^2}{|1-0.8|^2}=56.25,$$

$$R_x(e^{j\pi})=\frac{|1-0.5|^2}{|1+0.8|^2}=\frac{0.25}{3.24}\approx0.07716.$$

Their ratio is $56.25/0.07716=729$. Since $|0.8|<1$, the causal model is stable; since $|-0.5|<1$, it is minimum phase. **Numerically, the pole/zero pair is $(0.8,-0.5)$, the first samples are $[1,1.3,1.04]$, the endpoint PSDs are $56.25$ and $0.07716$, and the contrast is $729:1$.**
...
An active-sonar range cell contains a direct return plus one delayed bottom echo. Its estimated autocorrelation is $r(0)=5$, $r(\pm1)=2$, and $r(l)=0$ for $|l|>1$. Which lowest-order model family matches this signature? Find a minimum-phase model coefficient and its driving-noise variance, then calculate the PSD at $\omega=0$ and $\omega=\pi$.

Pole-zero equations alone do not tell an engineer how to recognize a model family from measured ACS, partial ACS, and spectrum. Section 3 provides those observable signatures.
...
$$\operatorname{corr}\lbrace x(n),x(n-2)\rbrace .$$
...
$$\operatorname{corr}\lbrace e_0(n),e_2(n)\rbrace .$$
...
**Question.** An echo cell has $r(0)=5$, $r(\pm1)=2$, and no correlation beyond lag one. Identify and recover its lowest-order minimum-phase model and endpoint PSDs.

**Answer.**

An autocorrelation sequence that cuts off after lag one is the signature of an MA(1) process,

$$x(n)=w(n)+b_1w(n-1).$$

Its correlations obey $r(0)=\sigma_w^2(1+b_1^2)$ and $r(1)=\sigma_w^2b_1$. The minimum-phase solution is

$$b_1=0.5,\qquad \sigma_w^2=4,$$

because $4(1+0.5^2)=5$ and $4(0.5)=2$. The alternative factor $b_1=2$, $\sigma_w^2=1$ has the same second-order statistics but is not minimum phase. For the selected factor,

$$R_x(e^{j\omega})=4|1+0.5e^{-j\omega}|^2.$$

Thus

$$R_x(e^{j0})=4(1.5)^2=9,\qquad
R_x(e^{j\pi})=4(0.5)^2=1.$$

**Numerically, the cell is MA(1), its minimum-phase echo coefficient is $0.5$, innovation variance is $4$, and its DC and Nyquist PSD values are $9$ and $1$.**
...
A hydrophone's reverberant background is believed to follow the ARMA(1,1) equation

$$x(n)+a_1x(n-1)=w(n)+b_1w(n-1).$$

Measurements give $r(0)=7$, $r(1)=4.5$, and $r(2)=2.25$. Use the modified Yule-Walker idea to recover $a_1$, then recover $b_1$ and the innovation variance from the autocorrelation of the AR-filtered residual. Finally calculate the modeled PSD at $\omega=0$ and $\omega=\pi$.

The signatures in §3 identify an ARMA family but do not estimate both its pole and zero. Section 4 separates the long-lag AR fit from the short-memory MA fit.
...
**Question.** Recover the ARMA(1,1) parameters from $r(0)=7$, $r(1)=4.5$, and $r(2)=2.25$, then calculate its PSD at $0$ and $\pi$.

**Answer.**

For lags beyond the MA order, $r(2)+a_1r(1)=0$. Hence

$$a_1=-\frac{2.25}{4.5}=-0.5.$$

Filter the data by $A(z)=1-0.5z^{-1}$ to obtain $u(n)=w(n)+b_1w(n-1)$. Its first two correlations, computed from those of $x$, are

$$r_u(0)=(1+0.5^2)r(0)-2(0.5)r(1)=1.25(7)-4.5=4.25,$$

$$r_u(1)=1.25r(1)-0.5[r(0)+r(2)]
=1.25(4.5)-0.5(9.25)=1.$$

These match an MA(1) process with

$$\sigma_w^2=4,\qquad b_1=0.25,$$

since $4(1+0.25^2)=4.25$ and $4(0.25)=1$. Therefore

$$R_x(e^{j0})=4\frac{1.25^2}{0.5^2}=25,\qquad
R_x(e^{j\pi})=4\frac{0.75^2}{1.5^2}=1.$$

**Numerically, $(a_1,b_1,\sigma_w^2)=(-0.5,0.25,4)$ and the endpoint PSD values are $25$ and $1$.**
...
For low-frequency vessel noise, a buoy estimates $r(0)=10$, $r(1)=6$, and $r(2)=2$ and chooses an AR(2) model. Using the autocorrelation/Yule-Walker method, find both denominator coefficients, the innovation variance, the two poles, and the PSD at $\omega=0$ and $\omega=\pi$. Is the estimated all-pole model stable?

Section 4 treats mixed ARMA estimation. Section 5 compares specialized AR estimators and shows how their fitted prediction polynomial becomes a stable high-resolution acoustic model.
...
**Question.** Fit an AR(2) model to $r(0)=10$, $r(1)=6$, and $r(2)=2$, and compute its poles, innovation variance, and endpoint PSDs.
...
$$a_1=-0.75,\qquad a_2=0.25.$$

The innovation variance is

$$\sigma_w^2=r(0)+a_1r(1)+a_2r(2)=10-4.5+0.5=6.$$

The roots of $z^2-0.75z+0.25=0$ are

$$z_{1,2}=0.375\pm j0.3307,$$

each of magnitude $0.5$, so the model is stable. Its PSD is $6/|1-0.75e^{-j\omega}+0.25e^{-j2\omega}|^2$, giving

$$R_x(e^{j0})=\frac6{0.5^2}=24,\qquad
R_x(e^{j\pi})=\frac6{2^2}=1.5.$$

**Numerically, the denominator coefficients are $(-0.75,0.25)$, innovation variance is $6$, poles are $0.375\pm j0.3307$, and endpoint PSDs are $24$ and $1.5$.**
...
An active sonar records a direct echo and two short delayed echoes. The estimated autocorrelation is $r(0)=5.25$, $r(1)=2.5$, $r(2)=1$, and $r(l)=0$ for $|l|\ge3$. Determine the minimum MA order and one causal minimum-phase shaping filter with its driving-noise variance. Give the two filter zeros and the PSD at $\omega=0$ and $\omega=\pi$.

AR methods exploit a long correlation tail; this measured correlation instead stops exactly. Section 6 explains how autocorrelation cutoff and spectral factorization identify a finite moving-average echo model.
...
**Question.** Given $r(0)=5.25$, $r(1)=2.5$, $r(2)=1$, and zero correlation from lag three onward, recover a minimum-phase finite echo model and its endpoint PSDs.

**Answer.**

The cutoff after lag two indicates MA(2). A spectral factor matching the correlations is

$$B(z)=1+0.5z^{-1}+0.25z^{-2},\qquad \sigma_w^2=4.$$

Indeed,

$$4(1^2+0.5^2+0.25^2)=5.25,$$

$$4[1(0.5)+0.5(0.25)]=2.5,\qquad 4[1(0.25)]=1.$$

The zeros solve $z^2+0.5z+0.25=0$, giving

$$z_{1,2}=-0.25\pm j0.4330,$$

both of magnitude $0.5$, so the factor is minimum phase. The endpoint PSDs are

$$R_x(e^{j0})=4(1+0.5+0.25)^2=12.25,$$

$$R_x(e^{j\pi})=4(1-0.5+0.25)^2=2.25.$$

**Numerically, the model is MA(2) with coefficients $(0.5,0.25)$, variance $4$, zeros $-0.25\pm j0.4330$, and endpoint PSDs $12.25$ and $2.25$.**
...
A narrowband underwater machinery component is represented by the AR(2) denominator

$$A(z)=1-0.9z^{-1}+0.81z^{-2},$$

with unit innovation variance and sampling rate $f_s=6\,\text{kHz}$. Find both poles and the physical frequency associated with their angle. Then calculate the parametric PSD at $\omega=\pi/3$, $0$, and $\pi$ and the ratio between the first and last values.

Estimating model coefficients is not yet the same as producing and interpreting a spectrum. Section 7 turns AR, MA, and ARMA parameters into numerical high-resolution PSD estimates and diagnostics.
...
**Question.** For $A(z)=1-0.9z^{-1}+0.81z^{-2}$, unit innovation variance, and $f_s=6\,\text{kHz}$, find the pole frequency and PSD values at $\pi/3$, $0$, and $\pi$.

**Answer.**

The denominator factors into conjugate poles of radius $0.9$ and angle $\pi/3$:

$$z_{1,2}=0.9e^{\pm j\pi/3}=0.45\pm j0.7794.$$

Their physical frequency is

$$f=\frac{\pi/3}{2\pi}(6000)=1000\ \text{Hz}.$$

With $R_x=1/|A|^2$, direct evaluation gives

$$R_x(e^{j\pi/3})=\frac1{0.0271}\approx36.900,$$

$$R_x(e^{j0})=\frac1{0.91^2}\approx1.208,$$

$$R_x(e^{j\pi})=\frac1{2.71^2}\approx0.1362.$$

The requested contrast is $36.900/0.1362=271$. **Numerically, the poles are $0.45\pm j0.7794$, their angle corresponds to $1\,\text{kHz}$, the three PSD values are $36.900$, $1.208$, and $0.1362$, and the first-to-last ratio is $271$.**
...
A monitoring system must choose compact models for two hydrophone backgrounds. Dataset A has $r_A(l)=9(2/3)^{|l|}$ for all lags. Dataset B has $r_B(0)=10$, $r_B(\pm1)=4$, and zero correlation for $|l|>1$. Identify the lowest-order AR or MA family for each dataset, recover every coefficient and innovation variance, and calculate each model's PSD at $\omega=0$ and $\omega=\pi$. Which model has infinite correlation memory and which has a one-lag cutoff?

The final section consolidates the chapter into a practical choice among model families and estimators; the preceding section alone does not yet make this mixed-resolution design decision.
...
**Question.** Dataset A has $r_A(l)=9(2/3)^{|l|}$; Dataset B has $r_B(0)=10$, $r_B(\pm1)=4$, and zero correlation beyond lag one. Find the lowest-order models, all parameters, and endpoint PSDs.

**Answer.**

Dataset A has a geometric correlation tail, the signature of AR(1):

$$x_A(n)=\frac23x_A(n-1)+w_A(n).$$

Its prediction polynomial coefficient is $a_1=-2/3$, and its innovation variance is

$$\sigma_{w_A}^2=9\left[1-\left(\frac23\right)^2\right]=5.$$

Thus

$$R_A(e^{j0})=\frac5{(1-2/3)^2}=45,\qquad
R_A(e^{j\pi})=\frac5{(1+2/3)^2}=1.8.$$

Dataset B cuts off after one lag, so it is MA(1). The minimum-phase factor is

$$x_B(n)=w_B(n)+0.5w_B(n-1),\qquad \sigma_{w_B}^2=8,$$

because $8(1+0.5^2)=10$ and $8(0.5)=4$. Hence

$$R_B(e^{j0})=8(1.5)^2=18,\qquad
R_B(e^{j\pi})=8(0.5)^2=2.$$

**Numerically, A is AR(1) with coefficient $2/3$, innovation variance $5$, and PSD endpoints $(45,1.8)$; B is MA(1) with echo coefficient $0.5$, innovation variance $8$, and endpoints $(18,2)$. A has infinite correlation memory, while B cuts off after one lag.**
